...
- A workflow run also binds the selected contract's `schema_version`, digest, declared route, and confidence floors into its checkpoint identity. If the contract file changes mid-run, `run event` and `run handoff` stop, while `run inspect` stays read-only and succeeds, returning `workflow_contract.stored`, `workflow_contract.current`, `workflow_contract.current_status` (`current`, `changed`, `missing`, `unloadable`, or `not-applicable`), `workflow_contract.drift_warning`, and `workflow_contract.drift_detail`. Inspection reads the run from its stored checkpoint, never re-validates the assignment against live contracts, and tolerates a failed contract lookup, so a deleted or invalid contract is reported rather than hidden behind an error; it also cannot be migrated until a valid contract is published again. Resume then requires an optional `workflow_contract_migration` object in the attestation naming `approval_id`, `stored_schema_version`, `stored_sha256`, `current_schema_version`, and `current_sha256`; it is accepted only when both attested identities match exactly, the workflow identity and route are unchanged, and no confidence floor is lowered.
