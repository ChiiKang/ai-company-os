...
- Handoffs carry evidence references, hashes/provenance, uncertainty, observable confidence basis, and individually scored load-bearing claims. `run handoff` advances only the next role declared by the original workflow, after current-role measurable success and medium-or-higher confidence for every load-bearing claim.
