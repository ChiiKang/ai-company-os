...
- `roles/`, `procedures/`, `workflows/`, and `schemas/` are public contracts; keep runtime behavior aligned with them, especially `ai-company.assignment.v1`. `workflows/*.json` is loaded and validated at runtime and is the only workflow authority; never restate a composition in Python.
